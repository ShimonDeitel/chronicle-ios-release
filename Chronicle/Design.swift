...
extension Color {
    static let chronAccent = Color(hex: "#007AFF")          // the single accent
    static let chronCard = Color(uiColor: .secondarySystemBackground)
    static let chronCard2 = Color(uiColor: .tertiarySystemBackground)
    static let chronField = Color(uiColor: .tertiarySystemFill)
    static let chronHair = Color(uiColor: .separator)
}

// MARK: - Flat surfaces (cards / pills / buttons)

extension View {
    func chronCard(cornerRadius: CGFloat = 20) -> some View {
        self.padding(16)
            .background(Color.chronCard, in: RoundedRectangle(cornerRadius: cornerRadius, style: .continuous))
    }

    func chronPill() -> some View {
        self.padding(.horizontal, 14).padding(.vertical, 8)
            .background(Color.chronCard, in: Capsule())
    }
...
struct FilledAccentButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .background(Color.chronAccent, in: Capsule())
...
struct ChronicleBackground: View {
